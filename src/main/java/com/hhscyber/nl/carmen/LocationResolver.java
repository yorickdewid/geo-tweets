...
import com.hhscyber.nl.carmen.types.Constants;
import com.hhscyber.nl.carmen.types.GeocodeLocationResolver;
import com.hhscyber.nl.carmen.types.Location;
import com.hhscyber.nl.carmen.types.ResolutionMethod;
import com.hhscyber.nl.carmen.utils.CarmenProperties;
 ...
 * getLocationResolver()).
 *
 * Given a tweet, resolveLocationFromTweet() will return a location for a tweet.
 * The location can be saved with the tweet.
 *
 * The returned location can be null if no location can be inferred for this
 * tweet. If a Location is returned, it can be a known or unknown location (see
 * below.)
 *
 * This class provides functionality to return the children (children contained
 * within location) and parent (location contained within parent) of a location.
 * Because the database does not know about all locations, the parent of a
 * location may not be known (but will be returned no matter what if requested.)
 * The children will only be returned if they are known. Since unknown objects
 * do no persist across different instantiations of the LocationResolver,
 * unknown locations should be handled with care.
 *
 * A NONE location represents the root of the location hierarchy.
 *
 * If use_unknown_places is true, then the LocationResolver will always return a
 * Location when the place field is set in a tweet. In this case, the Location
 * object may be missing information (e.g. county).
 *
 * If use_unknown_places is false, then a Location will only be returned when
 * the place field contains a known location.
 *
 * The carmen.properties file contains several options for controlling the
 * behavior of the LocationResolver. use_place: attempt to resolve based on the
 * tweet's place field. use_geocodes: attempt to resolve based on the tweet's
 * coordinates field. use_user_string: attempt to resolve based on the tweet's
 * user profile's location field. use_unknown_places: return places even if they
 * are not in the database. use_known_parent_for_unknown_places: if
 * use_unknown_places is false, then this option will try to find a known parent
 * for a location. If a known parent is found, and no location is found using
 * another method, the parent is used.
 *
 * The LocationResolver relies on resources specified in carmen.properties.
 *
 * locations: The database of locations in JSON format. place_name_mapping: A
 * mapping of Twitter place names to normalized forms. state_names_file: A list
 * of US states. country_names_file: A list of known countries
 *
 *
...
public class LocationResolver {
    ...
    public Location resolveLocationFromTweet(Map<String, Object> tweet) {
        ...
        if (location == null && this.useTimeZone) {
            location = resolveLocationUsingTimeZone(tweet);
            if (location != null) {
...
//	}
    ...
    /**
     * If a location object was created and saved, it may be unknown to this
     * LocationResolver. This method returns the known version of the Location
     * or registers it as a new location.
     *
     * @param location
     * @return
     */
